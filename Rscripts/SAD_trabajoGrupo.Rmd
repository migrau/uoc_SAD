...
```
2.  ¿existe prevalencia de una mutacion sobre un subtipo? 
...
>local({.Table <- with(TRAB_GRUP, table(SUBTIPO))cat("\ncounts:\n")print(.Table)cat("\npercentages:\n") print(round(100*.Table/sum(.Table), 2))})

```{r}
local({.Table <- with(TRAB_GRUP, table(SUBTIPO))cat("\ncounts:\n")print(.Table)cat("\npercentages:\n")print(round(100*.Table/sum(.Table), 2))})```
En nuestro estudio muestral se realizo un seguimiento a 282 paciente, en referencia al subtipo que presentan queda reflejado en la siguiente grafica.

> with(TRAB_GRUP, pie(table(SUBTIPO), labels=levels(SUBTIPO), xlab="", ylab="", main="SUBTIPO", col=rainbow_hcl(length(levels(SUBTIPO)))))

```{r}
with(TRAB_GRUP, pie(table(SUBTIPO), labels=levels(SUBTIPO), xlab="", ylab="", main="SUBTIPO", 
...
>local({
  .Table <- with(TRAB_GRUP, table(MUTACIONESPAÑA.DE.RESPAÑAISTENCIA)) cat("\ncounts:\n")print(.Table) cat("\npercentages:\n") print(round(100*.Table/sum(.Table), 2))})

```{r}
local({ .Table <- with(TRAB_GRUP, table(MUTACIONESPAÑA.DE.RESPAÑAISTENCIA))
cat("\ncounts:\n") print(.Table) cat("\npercentages:\n") print(round(100*.Table/sum(.Table), 2))})
```
...
> with(TRAB_GRUP, Barplot(MUTACIONESPAÑA.DE.RESPAÑAISTENCIA, 
...
Frequency table:
                                 SUBTIPO
MUTACIONESPAÑA.DE.RESPAÑAISTENCIA 01_AE 02_AG 06_CPX 14_BG 36_CPX  A1   B   C   D  F1   G   J
...
local({ .Table <- xtabs(~MUTACIONESPAÑA.DE.RESPAÑAISTENCIA+SUBTIPO, data=TRAB_GRUP)
 cat("\nFrequency table:\n") print(.Table) .Test <- chisq.test(.Table, correct=FALSE)print(.Test)})

```{r}
local({  .Table <- xtabs(~MUTACIONESPAÑA.DE.RESPAÑAISTENCIA+SUBTIPO, data=TRAB_GRUP)  cat("\nFrequency table:\n")  print(.Table)  .Test <- chisq.test(.Table, correct=FALSE)  print(.Test)})
```
